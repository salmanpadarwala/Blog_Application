...
const addBooks = async (req, res) => {
  try {
    ...
    const q =
      "INSERT INTO `bg_books_detail`( `book_title`, `book_author`, `book_publish_date`, `book_publish_time`, `book_description`, `book_thumbnail`, `book_pdf`,  `book_isdownload`, `books_category`) VALUES (?,?,?,?,?,?,?,?,?) ";
...
const getCateFilter = (req, res) => {
  ...
  const sql = `SELECT * FROM bg_books_detail WHERE books_category =? AND book_status= 1 LIMIT 4`;
...
const EditBooks = async (req, res) => {
  try {
    ...
    const q =
      "UPDATE `bg_books_detail` SET  `book_title`=?, `book_author`=?, `book_publish_date`=?, `book_publish_time`=?, `book_description`=?, `book_thumbnail`=?, `book_pdf`=?, `book_isdownload`=?, `books_category`=?  WHERE id = ?";
