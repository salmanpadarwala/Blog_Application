...
const NameCategory = () => {
  ...
  const deleteBlogCate = () => {
    if (selectedBlogId) {
      deleteBlogCategory(selectedBlogId);
      closeDeleteModal();
    }
  };
  const deleteBlogCategory = async (id) => {
    try {
      await axios.delete(`${PORT}deletenamescategory/${id}`);
  ...
  //ADD NAME CATEGORY DATA
  const savedata = async (e) => {
    e.preventDefault();
    const team = {
      categoryName: categoryName,
      categoryDesc: categoryDesc,
      subCategory: subCategory === "null" ? -1 : subCategory,
    };
    try {
      await axios
        .post(`${PORT}addnamescategory`, team)
        .then((res) => {
    ...
    <>
      ...
      <div className="relative dashboard px-5 mt-8">
        <div className="flex align-center">
          ...
          <span className="font-bold ml-3 text-2xl pt-1">Name Category</span>
        </div>
